...
int main(int argc, char *argv[]) {
    ...
    ini_ponto = (t_ponto *)malloc(sizeof(t_ponto)); // aloca espaço da struct t_ponto.
    if(ini_ponto == NULL)
        exit(1);
    proximo_ponto = ini_ponto; // valor do proximo ponto é igualado ao inicial.
    ...
    while(1) {
        ...
        printf("Deseja continua? <1> SIM <2> NAO: ");
        scanf("%d", &resp);
        if(resp == 1) {
